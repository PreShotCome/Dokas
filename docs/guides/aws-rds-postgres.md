# AWS RDS / Aurora PostgreSQL → Selket

Goal: produce a logical `pg_dump` of your RDS (or Aurora) PostgreSQL
database, then drill it in Selket for signed evidence.

> RDS automated **snapshots** are physical, not logical dumps — Selket drills
...
- `-Fc` is the custom format Selket restores (compressed, selective).
