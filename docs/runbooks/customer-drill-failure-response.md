...
**Audience:** whoever is on call for Dokaz.
...
verdict Dokaz gave them.

This is the most reputationally dangerous class of incident Dokaz has.
Dokaz exists to be trusted about whether a backup restores. A drill that
...
A correct FAILED verdict — Dokaz telling a customer their backup is
...
decide whether Dokaz is *correctly* reporting a broken backup. If it is,
the customer's backup is the problem and Dokaz did its job — pivot to
...
> Subject: Dokaz — investigating your drill on {target}
>
> Hi {name},
>
> We've seen the issue with your drill ({drill_id}) and a Dokaz engineer
