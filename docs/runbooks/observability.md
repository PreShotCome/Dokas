...
- Spans are tagged with `service.name=selket` and the deployment
...
- `dashboards/selket.json` — the service dashboard: HTTP request
...
- `dashboards/provisioning/` — Grafana provisioning: a Prometheus
  ...
  `/etc/grafana/provisioning/`, mount `selket.json` into
...
2. In production with `otlp`, run a drill and confirm the trace appears in
   Tempo/Grafana under `service.name = selket`.
3. `curl` `/metrics` (with the bearer token if `METRICS_TOKEN` is set) and
   confirm Prometheus is scraping it.
4. With the provisioning configs mounted, the "Selket — Service"
