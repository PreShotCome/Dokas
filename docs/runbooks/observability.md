...
- Spans are tagged with `service.name = dokaz` and the deployment
...
- `dashboards/dokaz.json` — the service dashboard: HTTP request
...
- `dashboards/provisioning/` — Grafana provisioning: a Prometheus
  ...
  `/etc/grafana/provisioning/`, mount `dokaz.json` into
