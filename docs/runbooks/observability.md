...
- Spans are tagged with `service.name = vesta` and the deployment
...
- `dashboards/vesta.json` — the service dashboard: HTTP request
...
- `dashboards/provisioning/` — Grafana provisioning: a Prometheus
  ...
  `/etc/grafana/provisioning/`, mount `vesta.json` into
